...
plate_yolo = YOLO(model_path=os.path.join(HOME_DIR,'./DLCV/DLCV_Colab_SrcCode_20200905/Detection/keras-yolo3/snapshots/000/plate_model.h5'),
...
img = Image.open(os.path.join(HOME_DIR, './DLCV/data/image/1212.jpg'))
...
plt.axis('off')
plt.savefig('./result/detected.jpg')
##OCR
import easyocr
reader = easyocr.Reader(['ko'])
#절삭 하자

result = reader.readtext('./result/detected.jpg', detail = 0)

str_result = "\n".join(result)

# plt.show()
with open("./result/번호판.txt", "w") as f:
    f.write(str_result)
    f.write('\n')
